...
export async function POST(request) {
    try {
        ...
        // check if user have already resistered a store
        const store = await prisma.store.findFirst({
            where: {userId: userId}
        })

        // if store is already registered then send the staus of the store
        if(store){
            return NextResponse.json({status: store.status})
        }

        // check is the usrename is already taken
        ...
        return NextResponse.json({status: "applied, waiting for the approval"})
    } catch (error) {
        console.error(error);
